...
**** Setting up ~yasnippet~                                 :B_ignoreheading:
     :PROPERTIES:
...
└── snippets
   ...
   Feel free to add or retrieve (a lot!) more snippets, i.e. to add more files within the ~ess-r-mode~ sub-directory.

**** Using ~yasnippet~ in an ESS[R] buffer                  :B_ignoreheading:
     :PROPERTIES:
     :BEAMER_env: ignoreheading
     :END:
